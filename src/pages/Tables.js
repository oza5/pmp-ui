...
import { Typography, List, ListItem, ListItemText, Tabs, Tab } from '@mui/material';
...
const Tables = () => {
  ...
  const location = useLocation();
    ...
    const tabData = useMemo(()=>[
    {
      ...
      items: [
        ...
        { id: 'Hypertension', name: 'Hyper tension', details: 'Details for Item 2' },
  ...
  useEffect(() => {
    ...
    const tabValue = tabData.findIndex((tab) => pathname.includes(`/details/${tab.items[0].id}`));
    if (tabValue !== -1) {
      ...
      {tabData.map((tab, index) => (
        <TabPanel key={index} value={activeTab} index={index}>
          <div className="tab">
            <List>
              {tab.items.map((item) => (
                <ListItem key={item.id} button component={Link} to={`/details/${item.id}`}>
